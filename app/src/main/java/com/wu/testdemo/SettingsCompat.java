...
public class SettingsCompat {
    ...
    public static void manageDrawOverlays(Activity context, int requestCode) {
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                try {
                    ...
                    goDefalut(context, requestCode);
                }
                return;
            }
            if (RomUtil.isVivo() && Build.VERSION.SDK_INT >= Build.VERSION_CODES.N_MR1) {
                try {
            ...
            Log.e("manageDrawOverlays", e.getMessage());
        }
    }

    public static void goDefalut(Activity context, int requestCode) {
        try {
            ...
            Intent intent = new Intent(Settings.ACTION_MANAGE_OVERLAY_PERMISSION);
            context.startActivityForResult(intent, requestCode);
            Toast.makeText(context, "跳转失败，请手动进入设置或安全中心！", Toast.LENGTH_SHORT).show();
        }
    }
